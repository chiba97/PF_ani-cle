...
このアプリはペットを飼っている方達が、孤独にならずにペット仲間を増やしたり、ペットとの思い出を共有したり、情報交換のできる楽しい場を作る為に制作いたしました。他のSNS等でペットアカウントを探す手間を省く為に、ペット関連専門のSNSサイトを考えました。昨今はペットを家族のように迎える方が増えているので、幅広い年代のユーザーが使用できるよう、シンプルで直感的な操作ができるデザインを意識しました。また、様々なデバイスで外出先でも使っていただきたく全ページにレスポンシブ対応を施しました。
